...
export default function Materials() {
  ...
  const handleResetMaterials = async () => {
    const confirmed = confirm(
      'سيتم حذف جميع بيانات إدارة المواد من قاعدة البيانات: المواد، التصنيفات، الوحدات، وحركات المخزون والمشتريات والعناصر المرتبطة بالمواد. لا يمكن التراجع عن هذا الإجراء. هل تريد المتابعة؟'
    );
    if (!confirmed) return;
    ...
    try {
      ...
      setImportMessage({
        type: 'success',
        text: 'تم تصفير بيانات إدارة المواد بالكامل. يمكنك الآن استيراد ملف Excel من جديد.',
      });
    } catch (err) {
      setImportMessage({
        type: 'error',
        text: 'فشل تصفير البيانات: ' + (err as Error).message,
      });
    } finally {
      setImporting(false);
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3">
        <div>
          <div className="font-semibold text-red-800">تصفير بيانات إدارة المواد</div>
          <div className="text-sm text-red-700">يحذف البيانات القديمة والمخفية المرتبطة بالمواد ثم يترك الورقة جاهزة للاستيراد من جديد.</div>
        </div>
        <button
          onClick={handleResetMaterials}
          className="btn bg-red-600 text-white hover:bg-red-700 shrink-0"
          disabled={importing}
          title="حذف جميع بيانات إدارة المواد القديمة"
        >
