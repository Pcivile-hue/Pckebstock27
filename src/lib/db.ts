...
// حذف جميع بيانات ورقة إدارة المواد فقط.
// حذف المواد يؤدي تلقائياً إلى حذف حركات المخزون والمشتريات وعناصر الاستهلاك المرتبطة بها
// وفق قيود قاعدة البيانات ON DELETE CASCADE، ثم نحذف التصنيفات والوحدات القديمة.
export async function resetMaterialsData(): Promise<void> {
  const { error: materialsError } = await supabase.from('materials').delete().not('id', 'is', null);
  if (materialsError) throw materialsError;

  const { error: categoriesError } = await supabase.from('categories').delete().not('id', 'is', null);
  if (categoriesError) throw categoriesError;

  const { error: unitsError } = await supabase.from('units').delete().not('id', 'is', null);
